...
def train(model: AlphaZeroNet, game_data, batch_size=32, lr=0.001, checkpoint_dir="checkpoints"):
    ...
    while curr_patience < patience:
        ...
        for i in range(0, len(game_data), batch_size):
            ...
            # Compute loss
            loss = calc_loss(policy_pred, value_pred, policies, outcomes)
...
num_iter = 10
lookback = 3
in_channels = lookback * 2 + 1
num_actions = 9 * 9 + 1

model = AlphaZeroNet(in_channels, num_actions)

for i in range(num_iter):
    print(f"starting self play {i}...")
    game_data = self_play(model, num_games=num_games, look_back=lookback)
    print(f"starting train {i}...")
    train(model, game_data, batch_size=32, lr=0.01, checkpoint_dir=f"checkpoint_dir_{iter}")
